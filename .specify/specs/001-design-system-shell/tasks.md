...
| ID | Task | Refs | File | Blocked by |
|---|---|---|---|---|
| T047 ✅ | **Mount the shell in the frontend layout.** HeaderNav, MobileTabBar, MenuSheet and Footer are built, tested, and render NOWHERE — grepping the layout for them returns 0, so FR-008's responsive shell exists as components nobody instantiates. Same family as FR-009's `profileHref`: built, never wired. Import through `@fablab/ui` (the public surface, which is why T039 comes first), wrap `{children}`, and assert the rendered layout carries the nav landmarks | FR-008, US3 | `apps/web/app/(frontend)/layout.tsx` | T039 |
| T048 ✅ | **A route behind every tab, so navigation does not dead-end.** Measured: only `/` and `/workbench` exist, while the nav points at seven destinations plus `/login` — every one a 404. Feature 003 owns the real pages (spec scope boundary), so these are deliberately minimal stubs that render the page name through the design system and nothing else. The test derives the expected route set from `tabs.ts` plus `LOGIN_HREF`, skipping entries flagged external, so a tab added later cannot ship without a destination | FR-008, US3 | `apps/web/app/(frontend)/*/page.tsx` | T047 |
| T049 ✅ | **Make the dev database resolve `localhost`.** The site 404s end-to-end today and the tenancy layer is not at fault: three organizations exist (test fixtures), so the sovereign fallback — which needs exactly one — is off, nothing carries a `localhost` domain, and the layout correctly turns that into `notFound()`. Measured by driving the app: `GET / -> HTTP 404`. Add `localhost` to the CITe organization's `domains` array in the seed, and document the one-line SQL for a dev DB the test suite has already polluted. **Do NOT touch resolution logic** — it behaved exactly as specified. Acceptance is a real `curl -s -o /dev/null -w '%{http_code}' localhost:3000/` returning 200 | FR-003, US2 | `apps/web/seed/index.ts`, `README.md` | — |
...
| T046 | **Add the exact context string `Colour tokens` to the required status checks on `main` and `dev`.** That is the job's `name:` in `ci.yml` (job key `colour-tokens`); protection matches the check-run NAME, not the key, and a wrong string never reports — which per feature 000's own probe blocks every PR indefinitely. Protection lists 11 contexts by name and a new job joins none of them automatically — until this lands the job is advisory, however red it goes. **Needs repo-admin rights: this is the user's action, not a commit** | SC-010 | branch protection (no file) | T045b |
