...
| CHK005 | FR-005: typography | [feasibility] Licence status is stated per face, and the requirement distinguishes the right to **use** from the right to **redistribute** — the two come apart here. Resolved by CLR-002 | [ ] |
...
| CHK027 | SC-012: renders without the restricted fonts | [testability] Building with no Aldo/Square binaries present is a runnable check **and the permanent CI condition**, so the fallback path cannot rot unnoticed | [ ] |
| CHK028 | CLR-002 consequence | [clarity] Production font delivery is named as a **feature 007 deployment concern**, not silently assumed — feature 001 ships `@font-face` pointing at a path it does not populate | [ ] |
