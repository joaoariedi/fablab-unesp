...
| T016b ✅ | **CLOSED by decision D1 (2026-09-06), not by implementation** — `clientUploads` is off, so there is no presigned path to cap per field and the limit is Payload's own. The row read "DEFERRED, blocked on a PO decision" for three days after the decision that closed it; § *T016b is a decision* below records the argument, which is still worth reading. Originally: Per-field size caps at presign. Moved out of the critical path after run 3 so the remaining 28 tasks are not held behind an architecture trade-off. See § "T016b is a decision" below | FR-012, SC-007 | — | **decision** |
