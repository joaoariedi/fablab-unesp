 ...
 *
 * 1. ~~**`projeto` has no author.**~~ **Closed.** `Projeto.ts` deferred `autor` → `perfilMaker`
 *    to feature 005 because Payload throws at config load for a relationship whose target does
 *    not exist; 005's T039 landed the field (nullable on day one, FR-034) and T008 gave a
 *    profile its `nivel`, so the lab byline this page credited on every card is gone and
 *    {@link autorDe} reads the populated maker instead — name, `@handle` and their **real**
 *    level (FR-033, US10).
